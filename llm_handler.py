...
def wygeneruj_temat_notatki(tekst: str) -> str:
    ...
    try:
        prompt = (
            "Na podstawie poniższego tekstu, wymyśl krótki, zwięzły tytuł (2-5 słów), "
            "który podsumowuje jego główny temat. Zwróć tylko i wyłącznie sam tytuł, "
            "bez żadnych dodatkowych zdań i cudzysłowów.\n\n"
            f"Tekst: \"{tekst[:1500]}\""
        )
        completion = client.chat.completions.create(
            model=getattr(config, "LLM_MODEL_NAME", "local-model"),

...
def przetworz_fragmenty_wstepnie(fragmenty: list[str], prompt_uzytkownika: str) -> (str, bool):
...
def dokonaj_finalnej_syntezy(polaczone_notatki: str) -> (str, bool):
    ...
    if len(tokeny_notatek) < config.CHUNK_TOKEN_LIMIT:
        ...
        polaczone_notatki_poziom2, success = przetworz_fragmenty_wstepnie(fragmenty_notatek, prompt_rekurencyjny)
        if not success:
